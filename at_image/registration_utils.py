...
def get_affine_grid(um_sizes, reg_params_dict, desired_res=1):
    """
    Generates a grid that has been transformed by the specified affine registration parameters.

    This function creates a grid at a desired resolution, then applies an affine transformation 
    based on the provided registration parameters. The affine transformation is defined by linear
    transformation (rotation, scaling, shear) and translation components.

    Parameters:
    um_sizes (list or tuple): The size of the grid in micrometers, specified as [height, width].
    reg_params_dict (dict): A dictionary containing the registration parameters. Expected keys are:
                            'a11', 'a21', 'a31', 'a12', 'a22', 'a32' (components of the linear
                            transformation matrix), and 'reg_x', 'reg_y', 'reg_z' (translation components).
    desired_res (float, optional): The desired resolution of the grid. Defaults to 1.

    Returns:
    ndarray: A numpy array representing the transformed grid. The shape of the array is determined by
             the input `um_sizes` and `desired_res`.

    Note:
    The grid is initially created in a two-dimensional space (height x width), and then the affine
    transformation is applied to this grid.
    """
